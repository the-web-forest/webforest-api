...
import GetVolunteerByIdUsecaseOut from "../controller/dtos/get.volunteer.by.id.output";
...
@Injectable()
export default class GetVolunteerByIdUseCase implements IUseCase<GetVolunteerByIdUseCaseInput, GetVolunteerByIdUsecaseOutput> {
    ...
    async run(input: GetVolunteerByIdUseCaseInput): Promise<GetVolunteerByIdUsecaseOutput> {
        this.logger.log('Starting')

        const volunteer = await this.volunteerRespository.findOne({ where: { id: input.id, isDeleted: false } });
