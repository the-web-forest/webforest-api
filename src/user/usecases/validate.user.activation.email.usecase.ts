...
export default class ValidateUserActivationEmailUseCase implements IUseCase<ValidateUserActivationEmailUseCaseInput, ValidateUserActivationEmailUseCaseOutput> {
    ...
    async run(input: ValidateUserActivationEmailUseCaseInput): Promise<ValidateUserActivationEmailUseCaseOutput> {
        this.logger.log('Starting')

        const activationRequest = await this.activationRequestRepository.findOne({ where: { hash: input.hash, activatedAt: IsNull() }, relations: ['user', 'user.roles'] })
    ...
    private async activateUser(activationRequest: ActivationRequest): Promise<void> {
