...
describe('User Login Use Case', () => {
  ...
  beforeEach(async () => {
    ...
    usecase = module.get<
      IUseCase<UserLoginUseCaseInput, UserLoginUseCaseOutput>
    >(UserLoginUseCaseToken);
  ...
  it('should not login user with temp role', async () => {
    ...
    const tempRole = await roleRepository.findOne({
      where: { id: RolesEnum.TempUser },
    });
    user.roles.push(tempRole);
    await user.save();

    const input = new UserLoginUseCaseInput({
      email: user.email,
  ...
  it('should login user with correct password', async () => {
    ...
    jest.spyOn(argon2 as any, 'verify').mockResolvedValue(true);
  ...
  it('should not login user with incorrect password', async () => {
    ...
    jest.spyOn(argon2 as any, 'verify').mockResolvedValue(false);
