  ...
  implements IUseCase<UserLoginUseCaseInput, UserLoginUseCaseOutput>
{
  ...
  async run(input: UserLoginUseCaseInput): Promise<UserLoginUseCaseOutput> {
    ...
    if (!user) {
      this.logger.warn(
        `User with email ${input.email} not found or not active`,
      );
    ...
    if (user.roles.some((role) => role.id === RolesEnum.TempUser)) {
      this.logger.warn(
        `User with email ${input.email} has not activated their account`,
      );
    ...
    if (!isPasswordValid) {
      this.logger.warn(
        `Invalid password attempt for user with email ${input.email}`,
      );
      throw new UserNotFoundError();
    }
  ...
  private calculateTokenExpirationDate(): Date {
    ...
    expirationDate.setHours(
      expirationDate.getHours() + Config.jwtExpirationTimeInHours,
    );
