...
export default class CreateNewsUseCase implements IUseCase<CreateNewsUseCaseInput, CreateNewsUseCaseOutput> {
    ...
    async run(input: CreateNewsUseCaseInput): Promise<CreateNewsUseCaseOutput> {
        return await this.newsRepository.save({ ...input, createdAt: new Date(), updatedAt: new Date() })
    }
